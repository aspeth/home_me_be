class Shelter
  ...
  def initialize(data)
    ...
    @shelter_type = data[:attributes][:TYPE]
    @sub_type = data[:attributes][:SUBTYPE]
    @number_of_beds = data[:attributes][:NUMBER_OF_BEDS]
    @lgbtq_focused = data[:attributes][:LGBTQ_FOCUSED].nil? ? "nil" : data[:attributes][:LGBTQ_FOCUSED]
    @ages_served = data[:attributes][:AGES_SERVED]
    @how_to_access = data[:attributes][:HOW_TO_ACCESS]
    @onsite_medical_clinic = data[:attributes][:ON_SITE_MEDICAL_CLINIC].nil? ? "nil" : data[:attributes][:ON_SITE_MEDICAL_CLINIC]
