...
RSpec.describe 'shelters api' do
  ...
  it 'sends info for one shelter by given id', :vcr do

    get "/api/v1/shelters/1"

    response_body = JSON.parse(response.body, symbolize_names: true)
    shelter = response_body[:data]
  ...
  it 'gives an error message if incorrect id is entered', :vcr do

    get "/api/v1/shelters/22"
  ...
  it 'gives an error message if no id is entered', :vcr do
